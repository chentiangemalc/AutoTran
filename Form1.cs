...
namespace AutoTran
{
    public partial class Form1 : Form
    {
        ...
        private void Worker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            var translated = translator.Translate("", Properties.Settings.Default.SourceLanguage, Properties.Settings.Default.DestinationLanguage);
            ...
            for (; ; )
            {
                ...
                this.BeginInvoke(new Action(() =>
                {
                    ...
                    // force to top
                    SetWindowPos(Handle, (IntPtr)SpecialWindowHandles.HWND_TOPMOST, 0, 0, 0, 0, SetWindowPosFlags.SWP_NOMOVE | SetWindowPosFlags.SWP_NOSIZE);
                    if (handle != IntPtr.Zero)
                    {
                        SetWindowPos(handle, IntPtr.Zero, (int)pos.X+Properties.Settings.Default.XOffset, (int)pos.Y+Properties.Settings.Default.YOffset, 0, 0, SetWindowPosFlags.SWP_NOZORDER | SetWindowPosFlags.SWP_NOSIZE | SetWindowPosFlags.SWP_SHOWWINDOW);
                    }

                    if (translated.IsCompleted)
                    {
                        if (!translated.IsFaulted)
                        {
                            if (string.IsNullOrEmpty(translated.Result))
                            {
                                label1.Text = "";
                            }
                            else
                            {
                                if (label1.Text != translated.Result)
                                {
                                    label1.Text = translated.Result;
                                }
                            }
                        }
                    }
                ...
                try
                {
                    AutomationElement el = AutomationElement.FromPoint(pos);
                    if (translated.IsCompleted)
                    {
                        if (el != null)
                        {
                            var propertyName = (from p in el.GetSupportedProperties() where Automation.PropertyName(p) == "Name" select el.GetCurrentPropertyValue(p)).FirstOrDefault().ToString();

                            if (!string.IsNullOrEmpty(propertyName))
                            {
                                translated = translator.Translate(propertyName, Properties.Settings.Default.SourceLanguage, Properties.Settings.Default.DestinationLanguage);
                ...
                {
                    Trace.WriteLine($"AUTOTRAN: {ex.Message}");
